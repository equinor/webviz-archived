...
class Theme(_Theme):
    ...
    Webviz exposes a set of ``jinja2`` macros that set up includes the content.
    A minimal example of a theme is as follows:
    ...
    :param css_files: List of additional `css` files to be included on each page.
    :param js_files: List of additional `js` files to be included on each page.
