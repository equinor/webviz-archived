...
export const arbitrarySizedArray = (arb, size) => jsc.tuple(new Array(size).fill(0).map(() => arb))

export const arbitraryCell = arbitrarySizedArray(jsc.number, 14).smap(
    ([c1, c2, c3, c4, c5, c6, c7, c8,
        i, j, fx0, fx1, fy0, fy1]) => new Cell(
        [[c1, c2], [c3, c4], [c5, c6], [c7, c8]],
        i, j, fx0, fy0, fx1, fy1,
    ),
    c => c.corners.values().concat([c.i, c.j])
...
export const arbitraryNonEmptyCell = jsc.suchthat(
    arbitraryCell,
    (c) => {
...
export function arbitraryMatrix(maxm, maxn) {
    return arbitraryMatrixArray(maxm, maxn)
        .smap(l => new Matrix(l), m => Array.from(m).map(d => Array.from(d)))
}
...
export const arbitraryGrid = jsc.array(arbitraryNonEmptyCell)
    .smap(l => new Grid(distinctIndecies(l)), g => g.getCells())

export const arbitraryField = arbitraryGrid
    .smap(
        g => new Field(g),
        f => f.grid,
    )

export const arbitraryParticle = jsc.tuple([arbitraryPoint, arbitraryField])
    .smap(
