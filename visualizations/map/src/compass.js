...
export default class Compass extends Component {
    constructor(config = {}) {
        ...
        this.position = config.initialPosition ? config.initialPosition : { x: 0, y: 0 }
        this.initialRotation = config.initialRotation ? config.initialRotation : 0
    ...
    _dragStarted() {
        ...
        this.element.selectAll('polygon')
            .attr('fill', '#A75C7C')

        this.dragStartAngle = this._calculateAngleFromCoord(x, y)
    }

    _dragged() {
        const [x, y] = d3.mouse(this.element.node())

        this.dragAngle = this._calculateAngleFromCoord(x, y)

        this.rotationAngle = (this.rotationAngle + this.dragAngle - this.dragStartAngle) % 360
    ...
    applyTransform() {
        ...
        transform.addTransform('rotate', [-this.initialRotation + this.rotationAngle, 100, 100])
