...
describe('Vector', () => {
    jsc.property(
        ...
        list => isEqual(Array.from(new Vector(list)), list),
    )
...
describe('Matrix', () => {
    ...
    jsc.property(
        'Matrix gets size from input',
        arbitraryMatrix(),
        (mat) => {
            const list = matrixToArray(mat)
            const matrix = new Matrix(list)
            if (list.length > 0) {
                return matrix.length === list.length
    ...
    jsc.property(
        ...
        (matrix) => {
            ...
            if (matrix.length > 0 && matrix.rowLength > 0) {
                return trans.length === matrix.rowLength
    ...
    jsc.property(
        ...
        (m1, m2) => {
            ...
            return m1.multiply(m2).determinant()
