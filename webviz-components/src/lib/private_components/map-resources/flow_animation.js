...
export default class FlowAnimation {
     ...
     */
    constructor(canvas, minflow, maxflow, particleGenerator, numParticles, pixelScale) {
        ...
        this._colors = d3.range(85, 255, 10).map(j => `rgba(${j}, ${j}, ${j}, 1.0)`)
    ...
    _stepParticles() {
        ...
        this._particles.forEach((d, i) => {
            ...
            if (!d.abandoned) {
                const [newCell, newNormalPosition] = field.simulate(d.cell, d.normalPosition)
                if (newNormalPosition && distance(newNormalPosition, d.normalPosition) < 0.001) {
                    d.abandon()
                }
                if (!newCell) {
                    d.abandon()
                    ...
                    d.cell = newCell
                    d.normalPosition = newNormalPosition
                }
            }
            d.decay()
        })
