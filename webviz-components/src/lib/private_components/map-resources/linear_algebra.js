...
export class Vector {
    ...
    [Symbol.iterator]() {
        ...
        return {
            next() {
                return nextIndex < self._data.length
                    ? { value: self._data[nextIndex++], done: false }
                    : { done: true }
            },
        }
    }
...
export class Matrix {
     ...
     */
    constructor(data, unsafe = false, length = undefined, rowLength = undefined) {
        if (unsafe) {
            this._data = data
            this._length = length
        ...
        return {
            next() {
                if (nextIndex < self._data.length) {
                    const result = new Vector()
                    result._data = self._data[nextIndex++]
     ...
     */
    multiply(matrix2) {
        if (matrix2.length === 0) return new Matrix([])
        if (this.length === 0) return new Matrix([])
...
export function affineIndependent([v1, ...rest]) {
    return linearIndependent(rest.map(v2 => v2.minus(v1)))
}
